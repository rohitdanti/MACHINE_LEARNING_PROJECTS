...
# Features
X = pima_df.iloc[:,:8] # SUBSETS THE DATAFRAME FOR ALL ROWS AND 8 COLUMNS

# Target variable
y = pima_df.iloc[:,8]  # SUBSETS THE DATAFRAME FOR ALL ROWS AND ONLY 8TH COLUMN
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=1) # TO SPLIT THE DATA INTO TRAIN(80%) AND TEST(20%) DATA.
...
dtree_probs = clf.predict_proba(X_test) [:,1] # TO CONSIDER ONLY THE TRUE POSITIVE VALUES
