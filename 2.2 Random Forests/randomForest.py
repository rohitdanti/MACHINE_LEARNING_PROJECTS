...
df_rf = pd.read_csv('diabetes.csv') # Loading the csv FIle

df_rf.head() #printing first five columns
...
X =  df_rf.iloc[:, :8] # taking first 7 columns and all rows into a variable X
y =  df_rf.iloc[:, 8] # taking the 8th column of all rows into a variable y
print(X)
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=0) # using the train_test_split method with suggested values
...
rf_clf = RandomForestClassifier(criterion="gini", min_samples_leaf=1, min_samples_split=10, max_features='auto', random_state=1) # using RandomFOrestClassifier with suggested values

# Train and fit Random Forest Classifer
rf_clf.fit(X_train, y_train) # Fitting the model to the training data

# Predict the response for test dataset
y_pred = rf_clf.predict(X_test) # predicting values for Test Data
...
rf_y_true = y_test
#Calculating and setting the Accuracy, Precision, Recall and f1-Score in respective variables
...
print("Best Accuracy:", best_accuracy) #Printing the Best Accuracy score
print("Best k:", best_k_fold)#printing the k value which got us the best accuracy score