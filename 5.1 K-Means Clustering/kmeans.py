...
# customer_data = pd.read_csv('/content/gdrive/MyDrive/customers.csv')
...
print(cust_shape)
print('GRADESCOPE 1.1 - '+str(cust_shape))
...
#matrix = customer_data.corr(numeric_only=True)# YOUR CODE HERE
# matrix = customer_data.corr()# YOUR CODE HERE
...
minimumCorrelationCoefficient = matrix.abs().min().min()
leastCorrelatedCols = np.where(matrix.abs() == minimumCorrelationCoefficient) 
col_param_1 = matrix.index[leastCorrelatedCols[0][0]] # Put the name of the first column here - 1.2 of Gradescope tests
col_param_2 = matrix.index[leastCorrelatedCols[1][0]] # Put the name of the second column here - 1.3 of Gradescope tests
print('Gradescope 1.2 - '+col_param_1)
print('Gradescope 1.3 - '+col_param_2)


# What is the max positive value present in the matrix that is not equal to 1
matrix_max = matrix[matrix < 1].max().max()# 1.4 of Gradescope tests
matrix_max = round(matrix_max, 2)
print('GRADESCOPE 1.4 - '+str(matrix_max))
...
print('GRADESCOPE 1.5 - '+str(chosen_cluster))
...
print('GRADESCOPE 1.6 - '+str(max_centre))
